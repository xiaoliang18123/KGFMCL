...
def get_performance(user_pos_test, r, auc, f1, Ks):
    ...
    for K in Ks:
        ...
        map.append(average_precision(r, K))

    return {'recall': np.array(recall), 'precision': np.array(precision),
            'ndcg': np.array(ndcg), 'hit_ratio': np.array(hit_ratio), 'map': np.array(map), 'auc': auc, 'f1': f1}
    # return {'recall': np.array(recall), 'precision': np.array(precision),
    #         'ndcg': np.array(ndcg), 'hit_ratio': np.array(hit_ratio), 'map': np.array(map), 'auc': auc}
...
def test_one_user(x):
    ...
    f1 = 0
    if args.f1:
        f1 = F1(user_pos_test, test_items, rating)
    return get_performance(user_pos_test, r, auc, f1, Ks)
    # return get_performance(user_pos_test, r, auc,  Ks)


def test(model, users_to_test, user_dict, n_params):
    ...
    for u_batch_id in range(n_user_batchs):
        ...
        if batch_test_flag:
            # batch-item test
            n_item_batchs = n_items // i_batch_size + 1 if n_items % i_batch_size != 0 else n_items // i_batch_size
        ...
        #map_start = time()
        batch_result = pool.map_async(test_one_user, user_batch_rating_uid)
        #map_end = time()
        #print('map use time:%.5f' % (map_end - map_start))
