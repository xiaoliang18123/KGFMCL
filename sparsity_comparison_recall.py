...
# 设置每个子图的横坐标标签
axs[0].set_xticks(x)
axs[0].set_xticklabels(last_fm)
axs[0].plot(x, last_fm_mf_recall, label='MF')
axs[0].plot(x, last_fm_kgat_recall, label='KGAT')
axs[0].plot(x, last_fm_kgin_recall, label='KGIN')
axs[0].plot(x, last_fm_kgcl_recall, label='KGCL', color='SaddleBrown')
axs[0].plot(x, last_fm_kgrec_recall, label='KGRec', color='Black')
axs[0].plot(x, last_fm_kgcna_recall, label='KGCNA', color='purple')
axs[0].plot(x, last_fm_kgcncl_recall, label='KGFMCL', color='red')
axs[0].set_title('Last-FM')
...
# 设置每个子图的横坐标标签
axs[1].set_xticks(x)
axs[1].set_xticklabels(yelp2018)
axs[1].plot(x, yelp2018_mf_recall, label='MF')
axs[1].plot(x, yelp2018_kgat_recall, label='KGAT')
axs[1].plot(x, yelp2018_kgin_recall, label='KGIN')
axs[1].plot(x, yelp2018_kgcl_recall, label='KGCL', color='SaddleBrown')
axs[1].plot(x, yelp2018_kgrec_recall, label='KGRec', color='Black')
axs[1].plot(x, yelp2018_kgcna_recall, label='KGCNA', color='purple')
axs[1].plot(x, yelp2018_kgcncl_recall, label='KGFMCL', color='red')
...
# 设置每个子图的横坐标标签
axs[2].set_xticks(x)
axs[2].set_xticklabels(alibaba_fashion)
axs[2].plot(x, alibaba_fashion_mf_recall, label='MF')
axs[2].plot(x, alibaba_fashion_kgat_recall, label='KGAT')
axs[2].plot(x, alibaba_fashion_kgin_recall, label='KGIN')
axs[2].plot(x, alibaba_fashion_kgcl_recall, label='KGCL', color='SaddleBrown')
axs[2].plot(x, alibaba_fashion_kgrec_recall, label='KGRec', color='Black')
axs[2].plot(x, alibaba_fashion_kgcna_recall, label='KGCNA', color='purple')
axs[2].plot(x, alibaba_fashion_kgcncl_recall, label='KGFMCL', color='red')
axs[2].set_title('Alibaba-iFashion')
axs[2].set_xlabel('User Group')
axs[2].set_ylabel('recall@20')
axs[2].legend()

# 设置整体标题和布局
# fig.suptitle('Multiple Line Charts with Multiple Lines in Each Subplot')
plt.tight_layout()
plt.savefig('sparsity_comparison_recall.eps', format='eps')
plt.savefig('sparsity_comparison_recall.pdf', format='pdf')
# 显示图表
plt.show()
